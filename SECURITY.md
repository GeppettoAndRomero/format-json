...
`format-json` runs entirely in your browser. There is no server component and no
account system, so the JSON you paste, type, or open is never uploaded. Most classic
...
- The parsing/formatting logic, the service worker, and the PWA manifest.
- Anything that could send your JSON input off the device.
