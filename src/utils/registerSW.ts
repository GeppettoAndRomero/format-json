...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/format-json/sw.js, scope=/format-json/）。
    // scope がツールの全 locale ページ（/format-json/, /format-json/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/format-json/sw.js', {
      scope: '/format-json/',
    });
